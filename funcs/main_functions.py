...
def process_building_data(istat_data, envelopes_data, consumption_appliances, user_data):
    ...
    for i in istat.index:
        ...
        ############################# SURFACES ###############################
        building_data[i]['Surfaces']  = {}
        building_data[i]['Surfaces']["Outdoor"]  = archetype.surfaces
        building_data[i]['Surfaces']["Adiabatic"]  = archetype.AdSurf
        ...
        # SPOSTARE QUESTO IN read_istat_data e fare in modo che sia dipendente anche dal CoeffRiporto
...
def simulate_hvac(building_data, weather_data, **kwargs):
    ...
    for bd_k, bd_dict_info in pg(building_data.items()):
        ...
        surfaces_data = []
        ad_surfaces_data = []
        for s in bd_dict_info["Surfaces"]["Outdoor"]:
            surfaces_data.append(s)

        for s in bd_dict_info["Surfaces"]["Adiabatic"]:
            ad_surfaces_data.append(s)
        ...
        for s in ad_surfaces_data:
            const = {
                'IntCeiling': ceiling,
                 'IntFloor': ceiling,
                 'IntWall':internal_wall
            }[s[0]]

            ad_surf = SurfaceInternalMass(
            s[0],
            area=s[1],
            surface_type=s[0],
            construction=const,
            )
            surfaces_obj.append(ad_surf)
        ...
        #     "central": "Centralized",
        #     "individual": "Single",
        # }[services["sh.layout"].lower()]

        # sh_device = scenario_data["sh.device"] if scenario_data["sh.device"] is not None else services["sh.device"]

        # try:
        #     emitter = {
        #         "radiators": "High Temp Radiator",
        #         "underfloor": "Radiant surface",
        #         "low temperature radiators": "Low Temp Radiator",
        #         "fan coil": "Fan coil",
        #     }[services["sh.emitters"].lower()]
        # except KeyError:
        #     emitter = "Fan coil"

        # device = {
        #     "gas boiler": "Condensing Gas Boiler",
        #     "oil boiler": "Oil Boiler",
        #     "coal heater": "Coal Heater",
        #     "heat pump": "A-W Heat Pump",
        #     "wood stove": "Stove",
        #     "district heating": "District Heating",
        #     "electric heater": "Electric Heater",
        # }[sh_device.lower()]

        # if device in ["Condensing Gas Boiler"]:
        #     heating_system = device + ", " + layout + ", " + emitter
        # elif device in ["Oil Boiler", "Coal Heater"]:
        #     heating_system = device + ", " + layout + ", High Temp Radiator"
        # elif device in ["District Heating"]:
        #     heating_system = device + ", Centralized, " + emitter
        # elif device in ["Stove"]:
        #     heating_system = device
        # elif device in ["A-W Heat Pump"]:
        #     if emitter == "High Temp Radiator":
        #         emitter = "Low Temp Radiator"
        #     heating_system = device + ", " + layout + ", " + emitter
        # elif device in ["Electric Heater"]:
        #     heating_system = device
        # else:
        #     raise TypeError("Heating system type not allowed")

        # ### Cooling system choice
        # layout = "Single"

        # try:
        #     emitter = {
        #         "underfloor": "Radiant surface",
        #         "low temperature radiators": "Fan Coil",
        #         "fan coil": "Fan coil",
        #         "split air conditioning": "Fan Coil",
        #     }[services["sc.emitters"].lower()]
        # except KeyError:
        #     emitter = "Fan coil"

        # # If sh device has changed from the scenario, then the heat pump is set also for cooling
        # sc_device = scenario_data["sh.device"] if scenario_data["sh.device"] is not None else services["sc.device"]

        # device = {
        #     "heat pump": "A-W chiller",
        #     "split air conditioning": "A-A split",
        #     "none": "IdealLoad",
        # }[sc_device.lower()]

        # if device in ["A-A split", "IdealLoad"]:
        #     cooling_system = device
        # elif device in ["A-W chiller"]:
        #     cooling_system = device + ", " + layout + ", " + emitter
        # else:
        #     raise TypeError("Cooling system type not allowed")
        ...
        if model == 'MeanMonthly':
            ...
            df_res_res = df_res.resample('ME').sum()
            df_res_res.iloc[:,16:20] = df_res.iloc[:,16:20].resample('ME').mean()
    ...
    # comp_time = time.process_time() - tic
    print("[Building simulations concluded]") #.format(comp_time)
